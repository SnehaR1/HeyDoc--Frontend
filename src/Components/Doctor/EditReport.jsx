...
function EditReport() {
    ...
    const { report_id, patientInfo, selectedReport } = state || {}
    const doctor = useSelector(state => state.doctorauth.doc_id)
    const [next, setNext] = useState(false)
    const [currentReport, setCurrentReport] = useState(selectedReport)
    const [reportInfo, setReportInfo] = useState(selectedReport)
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await doctorapi.put(`report/${report_id}/`, reportInfo)
            ...
            <div>
                <form className="max-w-lg mx-auto mt-3 mb-4" onSubmit={handleSubmit}>
                    {next ? <><div className="mb-3">
                        ...
                        <div className=' flex-row'>    <button type="button" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 " onClick={() => setNext(false)}>Back</button>  <button type="submit" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Update</button></div>
                    </> :
